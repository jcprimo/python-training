...
def hanoi_solver_aux(tower, dep, arr, mid, solution):
	...
	if(tower_size) == 1:
		...
		soluion = hanoi_solver_aux(new_tower, 	mid, 	arr, 	dep, temp_solution)
	return solution
...
def play_solution(tower_size, speed):
	...
	#Here is where the soltuion begins
	...
	for i in range(solution_length):
		if i == solution_length - 1:
			move_and_print(2, solution_recipe, towers)
			...
			move_and_print(speed, solution_recipe, towers)


def move_and_print(speed, solution_recipe, towers):
	targeted_tower = towers_manipulation.translated_tower_index(solution_recipe[i][2])
	ring_move = [solution_recipe[i][0],targeted_tower]
	towers_manipulation.move_ring(ring_move,towers)
	print("\n")
	hanoi_ascii.print_towers(towers)
